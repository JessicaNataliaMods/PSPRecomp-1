...
req(profile, 'load_cancel_workaround', 'mode-aware cancellation missing')
req(profile, 'kPspUtilityDialogResultCancel = 1u', 'PSP CANCEL constant missing')
req(profile, 'kPspUtilityDialogResultAbort = 2u', 'LOAD ABORT compatibility constant missing')
...
if 'FORCE_V9_4_SAVEDATA_UI_OBJECTS.bat' not in build and 'FORCE_V9_5_HOST_TEXTURE_OBJECTS.bat' not in build:
    errors.append('savedata UI object invalidation not wired')
...
print(' - cancel is mode-aware: LOAD compatibility abort, SAVE/DELETE PSP cancel')
